...
class DinoFusion(nn.Module):
    ...
    def dino_feature(self, x):
        img_h, img_w = x.shape[-2:]
        
        # --- Encoder Forward Pass ---
        # Get intermediate features from the backbone
        intermediate_outputs = self.extractor.get_intermediate_layers(
            x, 
            n=[2,5,8,11], 
            reshape=False, # Keep as (B, N, C) for the head
            return_class_token=False,
        ...
        # Reshape features to [B, C, H, W]
        reshaped_features = []
        patch_h, patch_w = img_h // self.extractor.patch_size, img_w // self.extractor.patch_size
        for feat in intermediate_outputs:
            reshaped_features.append(feat.permute(0, 2, 1).reshape(x.shape[0], -1, patch_h, patch_w))
    ...
    def forward(self, A, B):
        ...
        x_dino = self.dino_feature(x)
        y_dino = self.dino_feature(y)
        return x_dino, y_dino


class Adapter(nn.Module):
    def __init__(self, embed_dim=96, dim=64):
    ...
    def forward(self, x, reshaped_features) -> torch.Tensor:
        f2, f5, f8, f11 = reshaped_features
        f8_11 = self.upconv1(f8 + f11)
        f5_up = self.upconv2(f5)
        f2_up = self.upconv3(f2)
...
class One_Preprocess(nn.Module):
    def __init__(self, img_size=64, patch_size=1, in_chans=3,
                 ...
                 use_checkpoint=False, upscale=2, img_range=1., upsampler='', extractor=None, resi_connection='1conv',
                 **kwargs):
        super(One_Preprocess, self).__init__()
        self.img_range = img_range
        self.window_size = 16
        print('in_chans: ', in_chans)
        if in_chans == 3 or in_chans == 6:
    ...
    def dino_ectractor(self, x):
        img_h, img_w = x.shape[-2:]
        intermediate_outputs = self.extractor.get_intermediate_layers(
            x, n=[2,5,8,11], reshape=False, return_class_token=False
        )
        reshaped_features = []
        patch_h, patch_w = img_h // self.extractor.patch_size, img_w // self.extractor.patch_size
        for feat in intermediate_outputs:
    ...
    def forward(self, A, B):
        ...
        reshaped_features_A = self.dino_ectractor(x)
        reshaped_features_B = self.dino_ectractor(y)
        return x, y, reshaped_features_A, reshaped_features_B


class Two_Adapter(nn.Module):
    def __init__(self, dino_dim, embed_dim=64):
        super(Two_Adapter, self).__init__()
        self.adapter_A = Adapter(dino_dim, embed_dim)
        self.adapter_B = Adapter(dino_dim, embed_dim)
        self.apply(self._init_weights)
    ...
    def forward(self, x, y, reshaped_features_A, reshaped_features_B):
        enc_x = self.adapter_A(x, reshaped_features_A)
        enc_y = self.adapter_B(y, reshaped_features_B)
        return enc_x, enc_y


class Three_Rec(nn.Module):
    def __init__(self, embed_dim=64):
        super(Three_Rec, self).__init__()
        self.rec_A = Reconstruction(embed_dim)
        self.rec_B = Reconstruction(embed_dim)
        self.apply(self._init_weights)
...
class Four_Fusion(nn.Module):
    def __init__(self, embed_dim=64):
        super(Four_Fusion, self).__init__()
        ...
        self.former_last = TransformerBlock(dim=embed_dim, num_heads=8, ffn_expansion_factor=2, bias=False, LayerNorm_type='WithBias')
        self.conv_last2 = nn.Conv2d(embed_dim, int(embed_dim//2), 3, 1, 1)
        self.conv_last3 = nn.Conv2d(int(embed_dim/2), 3, 3, 1, 1)
        self.apply(self._init_weights)
...
if __name__ == '__main__':
    ...
    one_preprocess = One_Preprocess(extractor=dinov3_block).cuda()
    two_adapter = Two_Adapter(dino_dim=dinov3_block.embed_dim, embed_dim=64).cuda()
    three_rec = Three_Rec(embed_dim=64).cuda()
    four_fusion = Four_Fusion(embed_dim=64).cuda()

    x, y, reshaped_features_A, reshaped_features_B = one_preprocess(x, y)
    enc_x, enc_y = two_adapter(x, y, reshaped_features_A, reshaped_features_B)
    rec_x, rec_y = three_rec(enc_x, enc_y)
    f = four_fusion(enc_x, enc_y)
    print(f.shape)
