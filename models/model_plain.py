...
class ModelPlain(ModelBase):
    ...
    def define_loss(self):
        ...
        if G_lossfn_type == 'l1':
            ...
            from models.loss_mef import fusion_loss_mef_rec
            self.G_lossfn = fusion_loss_mef_rec().to(self.device)
        elif G_lossfn_type == 'vif':
            from models.loss_vif import fusion_loss_vif
            from models.loss_vif import fusion_loss_vif_rec
            self.G_lossfn = fusion_loss_vif_rec().to(self.device)
        elif G_lossfn_type == 'mff':            
            from models.loss_mff import fusion_loss_mff
            from models.loss_mff import fusion_loss_mff_rec
            self.G_lossfn = fusion_loss_mff_rec().to(self.device)
            ...
            from models.loss_med import fusion_loss_med_rec
            self.G_lossfn = fusion_loss_med_rec().to(self.device)
    ...
    def optimize_parameters(self, current_step):
        ...
        if G_lossfn_type in ['loe', 'gt']:
            ...
            total_loss, loss_text, loss_int, loss_ssim, loss_rec = self.G_lossfn(self.A, self.B, self.E)
        ...
        if G_lossfn_type in ['loe', 'mef', 'vif', 'mff', 'gt', 'nir', 'med']:
            ...
            self.log_dict['Rec_loss'] = loss_rec.item()
        ...
        if G_lossfn_type == 'mef':
            ...
            self.writer.add_image('fused_image[0]', self.E[0][0])
            self.writer.add_image('fused_image[1]', self.E[0][-1])
        elif G_lossfn_type == 'vif':
    ...
    def current_visuals(self, need_H=False):
        ...
        out_dict['A_rec'] = self.E[1].detach()[0].float().cpu()
        out_dict['E_rec'] = self.E[2].detach()[0].float().cpu()
        # out_dict['A_E'] = self.E[3].detach()[0].float().cpu()
        # out_dict['B_E'] = self.E[4].detach()[0].float().cpu()
        if need_H:
