...
class Sobelxy(nn.Module):
    ...
    def forward(self,x):
        sobelx, sobely = torch.zeros_like(x), torch.zeros_like(x)
        for i in range(x.shape[1]):
            sobelx[:,i:(i+1),:,:]=F.conv2d(x[:,i:(i+1),:,:], self.weightx, padding=1)
            sobely[:,i:(i+1),:,:]=F.conv2d(x[:,i:(i+1),:,:], self.weighty, padding=1)
...
class L_rec(nn.Module):
    ...
    def forward(self, image_A, image_B, rec_A, rec_B):
        L_rec1 = F.l1_loss(image_A, rec_A)
        L_rec2 = F.l1_loss(image_B, rec_B)
        l_rec = 0.5*L_rec1+0.5*L_rec2
        return l_rec
...
class fusion_loss_mff_rec(nn.Module):
    def __init__(self):
        super(fusion_loss_mff_rec, self).__init__()
        self.L_Grad = L_Grad()
        self.L_Inten = L_Intensity()
        self.L_SSIM = L_SSIM()
        self.L_rec = L_rec()
        ...
        # print(1)
    def forward(self, image_A, image_B, image_fused):
        image_fused,rec_A,rec_B = image_fused
        loss_l1 = 20 * self.L_Inten(image_A, image_B, image_fused)
        loss_gradient = 20 * self.L_Grad(image_A, image_B, image_fused)
        loss_SSIM = 10 * (1 - self.L_SSIM(image_A, image_B, image_fused))
        loss_rec = 10 * self.L_rec(image_A, image_B, rec_A, rec_B)
        fusion_loss = loss_l1 + loss_gradient + loss_SSIM + loss_rec
        return fusion_loss, loss_gradient, loss_l1, loss_SSIM, loss_rec
